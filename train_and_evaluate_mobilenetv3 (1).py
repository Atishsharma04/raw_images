"""
train_and_evaluate_mobilenetv3.py
-----------------------------------
Trains and evaluates a lightweight MobileNetV3 classifier for building
damage assessment (No Damage / Minor Damage / Major Damage / Destroyed)
on cropped building images.

Matches this folder layout:

    <ROOT>\\train_images_labels_targets\\train\\sam_crops\\
        destroyed\\
        major-damage\\
        minor-damage\\
        no-damage\\
    ...
    <ROOT>\\test_images_labels_targets\\test\\sam_crops\\
        destroyed\\
        major-damage\\
        minor-damage\\
        no-damage\\
...
(stratified by class) since no separate val folder exists.
...
Usage:
    python train_and_evaluate_mobilenetv3.py ^
...
# --------------------------------------------------------------------------
def get_args():
    parser = argparse.ArgumentParser(description="Train + evaluate MobileNetV3 for building damage classification")
    ...
    parser.add_argument("--run_name", type=str, default="mobilenetv3_run")
    ...
    parser.add_argument("--model_variant", type=str, default="large", choices=["large", "small"])
...
def main():
    ...
    print(f"[model] Building MobileNetV3-{args.model_variant} (pretrained ImageNet)...")
